...
- **`workflow_artifacts`** — Artifact outputs produced by agent iterations (keyed by task, stage, and name)
...
Stores artifact outputs produced by agent iterations. Each row records a named artifact (e.g., `plan`, `summary`) for a given task, stage, and iteration. Supersedes the deprecated `artifact_snapshot` column on `workflow_iterations`.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | TEXT | PRIMARY KEY | Unique artifact identifier |
| `task_id` | TEXT | NOT NULL, FOREIGN KEY → workflow_tasks(id) | Associated task |
| `iteration_id` | TEXT | FOREIGN KEY → workflow_iterations(id) | Iteration that produced this artifact (nullable) |
| `stage` | TEXT | NOT NULL | Stage name that produced this artifact (e.g., `"planning"`, `"work"`) |
| `name` | TEXT | NOT NULL | Artifact name (e.g., `"plan"`, `"summary"`, `"breakdown"`) |
| `content` | TEXT | NOT NULL | Artifact content (plain text or JSON) |
| `created_at` | TEXT | NOT NULL | ISO 8601 timestamp |

**Indexes:**
- `idx_workflow_artifacts_task_id` on `task_id`
- `idx_workflow_artifacts_task_stage_name` on `(task_id, stage, name)`
...
workflow_iterations
    ...
    └─> workflow_artifacts (iteration_id)
