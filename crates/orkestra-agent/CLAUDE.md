...
- **Sidechain transcript format** — Claude Code writes subagent (Agent tool) internal calls to *separate* sidechain files at `<session-id>/subagents/agent-<id>/agent.jsonl`, not the main transcript. Each sidechain directory also has `meta.json` containing `toolUseId` (the parent Agent tool-use ID from the main transcript) and `agentType`. The PTY path polls for new sidechain directories every 150ms via `discover_new_sidechains()`, reads their JSONL, and **injects `parent_tool_use_id`** into each line before passing to the parser. This lets all existing subagent detection logic in `orkestra-parser` work without changes. Sidechain lines are deliberately excluded from `full_output` — only the main agent's output matters for structured output extraction.
...
- **UTF-8 truncation requires `str::get`, not byte-slice indexing**: When truncating PTY line content (e.g., for debug logging), use `line.get(..n).unwrap_or(line)` — not `&line[..n]`. PTY output can contain partial multi-byte UTF-8 sequences mid-write; indexing at an arbitrary byte offset panics if it falls inside a character. The same risk applies anywhere you byte-slice into a `&str` derived from PTY output.
- **50ms sidechain finalization heuristic**: `finalize_pty_session` sleeps 50ms before the final sidechain reads to let in-flight sidechain writes flush. If a sidechain file flushes slower than the main transcript (e.g., under load), trailing subagent tool calls may be missed. If you see incomplete subagent log entries at session end, check `finalize_pty_session`'s sleep and final `update_sidechains` call.
