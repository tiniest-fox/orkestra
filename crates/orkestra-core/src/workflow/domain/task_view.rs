...
pub struct DerivedTaskState {
    ...
    /// True when in `AwaitingApproval` and the stage has approval capability.
...
/// Check if the current stage's agent produced an approval verdict.
///
/// Returns `true` when the task is in `AwaitingApproval` and the current stage
/// has approval capability. This works because:
/// - Only approval-capability stages produce agent verdicts
/// - Rejections go to `AwaitingRejectionConfirmation`, not `AwaitingApproval`
/// - So `AwaitingApproval` + approval capability = agent approved
fn extract_pending_approval(task: &Task, workflow: &WorkflowConfig) -> bool {
...
mod tests {
    ...
    fn test_derived_state_with_questions() {
        ...
        let derived = DerivedTaskState::build(&task, &[iter], &[], &[], &test_default_workflow());
    ...
    fn test_derived_state_with_subtask_progress() {
        ...
        let sub1_derived = DerivedTaskState::build(&sub1, &[], &[], &[], &test_default_workflow());

        let sub2 = Task::new("sub-2", "Sub 2", "Desc", "work", "now");
        let sub2_derived = DerivedTaskState::build(&sub2, &[], &[], &[], &test_default_workflow());

        let mut sub3 = Task::new("sub-3", "Sub 3", "Desc", "work", "now");
        sub3.state = TaskState::failed("error");
        let sub3_derived = DerivedTaskState::build(&sub3, &[], &[], &[], &test_default_workflow());
    ...
    fn test_derived_state_pending_approval() {
        ...
        let mut task = make_task("review"); // review stage has approval capability
        task.state = TaskState::awaiting_approval("review");

        // No iteration outcome needed — config lookup handles it
        let iter = Iteration::new("iter-1", "task-1", "review", 1, "now");
        let derived = DerivedTaskState::build(&task, &[iter], &[], &[], &workflow);
    ...
    fn test_derived_state_no_pending_approval_when_not_awaiting() {
        let workflow = test_default_workflow();
        let mut task = make_task("review"); // review has approval capability
    ...
    fn test_task_view_serialization() {
        ...
        let view = TaskView {
            ...
            iterations: vec![],
            stage_sessions: vec![],
            derived: DerivedTaskState::build(&task, &[], &[], &[], &test_default_workflow()),
        };
