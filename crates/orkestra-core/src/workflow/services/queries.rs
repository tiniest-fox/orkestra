...
impl WorkflowApi {
    ...
    /// Get the line count for a named artifact.
    ///
    /// Returns the number of lines in the artifact (newline count + 1),
    /// or None if the artifact doesn't exist.
    ///
    /// # Arguments
    /// * `task_id` - The task ID
    ...
    ///
    /// # Returns
    /// `Some(line_count)` if the artifact exists, `None` otherwise
    ///
    /// # Errors
    /// Returns `TaskNotFound` if the task doesn't exist
    pub fn get_artifact_line_count(
        &self,
        task_id: &str,
        artifact_name: &str,
    ) -> WorkflowResult<Option<usize>> {
        let artifact = self.get_artifact(task_id, artifact_name)?;
    ...
    /// Get the total log entry count across all stage sessions for a task.
    ///
    /// # Arguments
    /// * `task_id` - The task ID
    ///
    /// # Returns
    /// Total number of log entries across all stages
    ///
    /// # Errors
    /// Returns `TaskNotFound` if the task doesn't exist
    pub fn get_task_log_count(&self, task_id: &str) -> WorkflowResult<usize> {
        ...
        let mut total = 0;
        for session in sessions {
            let logs = log_service.get_logs(&session.id)?;
            total += logs.len();
        }
...
mod tests {
    ...
    #[test]
    fn test_get_artifact_line_count() {
        let workflow = test_workflow();
        let store = Arc::new(InMemoryWorkflowStore::new());
        let api = WorkflowApi::new(workflow, store);
        ...
        // Create artifact with multiple lines
        let _ = api
            .process_agent_output(
                &task.id,
                StageOutput::Artifact {
        ...
        let count = api.get_artifact_line_count(&task.id, "plan").unwrap();
        assert_eq!(count, Some(3));

        // Test missing artifact
        let missing = api
            .get_artifact_line_count(&task.id, "nonexistent")
            .unwrap();
        assert_eq!(missing, None);
    }

    #[test]
    fn test_get_artifact_line_count_empty() {
        let workflow = test_workflow();
        let store = Arc::new(InMemoryWorkflowStore::new());
        let api = WorkflowApi::new(workflow, store);
        ...
        let empty_count = api.get_artifact_line_count(&task.id, "plan").unwrap();
        assert_eq!(empty_count, Some(0));
    }

    #[test]
    fn test_get_artifact_line_count_task_not_found() {
        let workflow = test_workflow();
        let store = Arc::new(InMemoryWorkflowStore::new());
        let api = WorkflowApi::new(workflow, store);

        let result = api.get_artifact_line_count("nonexistent-task", "plan");
        assert!(matches!(result, Err(WorkflowError::TaskNotFound(_))));
    }

    #[test]
    fn test_get_task_log_count() {
        use crate::workflow::domain::StageSession;
        ...
        // Initially no logs
        let count = api.get_task_log_count(&task.id).unwrap();
        assert_eq!(count, 0);

        // Create a stage session and add some log entries
        ...
        api.store
            .append_log_entry(
                &session.id,
                &LogEntry::Text {
                    content: "log message 1".to_string(),
                },
            )
            .unwrap();
        api.store
            .append_log_entry(
                &session.id,
                &LogEntry::Text {
                    content: "log message 2".to_string(),
                },
            )
            .unwrap();

        // Should count logs from all sessions
        let count = api.get_task_log_count(&task.id).unwrap();
        assert_eq!(count, 2);

        // Add a second stage session with more logs
        let session2 = StageSession::new(
            format!("{}-work", task.id),
            &task.id,
            "work",
            chrono::Utc::now().to_rfc3339(),
        );
        api.store.save_stage_session(&session2).unwrap();
        api.store
            .append_log_entry(
                &session2.id,
                &LogEntry::Text {
                    content: "log message 3".to_string(),
                },
            )
            .unwrap();
    ...
    #[test]
    fn test_get_task_log_count_task_not_found() {
        let workflow = test_workflow();
        let store = Arc::new(InMemoryWorkflowStore::new());
        let api = WorkflowApi::new(workflow, store);

        let result = api.get_task_log_count("nonexistent-task");
        assert!(matches!(result, Err(WorkflowError::TaskNotFound(_))));
    }
}
