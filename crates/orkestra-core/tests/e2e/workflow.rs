...
/// Test that `restart_on_reentry: true` creates a fresh session on pipeline re-entry.
///
/// Flow:
/// 1. Task goes through: work → review (approve) → integration
/// 2. Integration fails → task returns to work (pipeline re-entry, not rejection)
/// 3. Verify: new session created (different UUID), is_resume = false, full prompt sent
#[test]
#[allow(clippy::too_many_lines)]
fn test_restart_on_reentry_starts_fresh_session() {
    use orkestra_core::workflow::config::{StageCapabilities, StageConfig};
    use orkestra_core::workflow::domain::SessionState;

    // Build work stage with restart_on_reentry: true
    let work_stage = StageConfig::new("work", "summary")
        .with_prompt("worker.md")
        .restart_on_reentry();
    ...
    let task = ctx.create_task(
        ...
        "Test that restart_on_reentry creates fresh session",
        None,
    );
    let task_id = task.id.clone();
    ...
    // Verify first spawn is a full prompt (not resume)
    ctx.assert_full_prompt("summary", false, false);
    let first_call = ctx.last_run_config();
    assert!(!first_call.is_resume, "First spawn should not be a resume");

    // Record the original work session
    let original_session = ctx
        .api()
        .get_stage_session(&task_id, "work")
        .unwrap()
        .expect("Work session should exist");
    let original_session_id = original_session.id.clone();

    // Approve work → advances to review (automated)
    ctx.api().approve(&task_id).unwrap();

    // =========================================================================
    // Step 2: Review approves → integration begins
    // =========================================================================

    // Create a merge conflict to force integration failure
    let worktree_path = ctx.repo_path().join(".orkestra/.worktrees").join(&task_id);
    std::fs::write(
        worktree_path.join("conflict.txt"),
        "Task's version",
    )
    ...
    orkestra_core::testutil::create_and_commit_file_on_branch(
        ...
        "Add conflicting file on base",
    )
    .unwrap();

    // Queue review approval and recovery work output
    ctx.set_output(
        &task_id,
        MockAgentOutput::Approval {
            decision: "approve".to_string(),
    ...
    ctx.advance(); // spawns reviewer (completion ready)
    ctx.advance(); // processes review → integration fails → recovers to work → spawns work agent (completion ready)
    ctx.advance(); // processes work output

    // =========================================================================
    // Step 3: Verify fresh session on pipeline re-entry (after integration failure)
    // =========================================================================

    // Verify task is back at work stage
    let task = ctx.api().get_task(&task_id).unwrap();
    assert_eq!(
        task.current_stage(),
        Some("work"),
        "Task should be back at work stage after integration failure"
    );

    // Check spawn config: should NOT be a resume
    let reentry_call = ctx.last_run_config();
    assert!(
        !reentry_call.is_resume,
        "Pipeline re-entry with restart_on_reentry should NOT be a resume"
    );

    // Check prompt: should be full prompt (not resume marker)
    let reentry_prompt = ctx.last_prompt();
    assert!(
        !reentry_prompt.starts_with("<!orkestra:resume:"),
        "Pipeline re-entry should get full prompt, not resume prompt. Got: {}",
        &reentry_prompt[..reentry_prompt.len().min(200)]
    );

    // Check sessions: should have 2 work sessions (original + new)
    let all_sessions = ctx.api().get_stage_sessions(&task_id).unwrap();
    let work_sessions: Vec<_> = all_sessions.iter().filter(|s| s.stage == "work").collect();
    assert_eq!(
        work_sessions.len(),
        2,
        "Should have 2 work sessions (original + fresh on re-entry). Got: {work_sessions:?}"
    );

    // Find the superseded and new sessions
    let superseded = work_sessions
        .iter()
        .find(|s| s.session_state == SessionState::Superseded)
        .expect("Should have a Superseded session");
    let new_session = work_sessions
        .iter()
        .find(|s| s.session_state == SessionState::Active)
        .expect("Should have a new Active session");

    // Verify the original session was superseded
    assert_eq!(
        superseded.id, original_session_id,
        "Original session should be the one marked Superseded"
    );

    // Verify new session has a different UUID
    assert_ne!(
        new_session.id, original_session_id,
        "Fresh session should have a different UUID"
    );

    // Verify new session has a different claude_session_id
    assert_ne!(
        new_session.claude_session_id, original_session.claude_session_id,
        "Fresh session should have a different claude_session_id"
    );
}

/// Regression test: without `restart_on_reentry`, stage re-entry should use recheck resume.
///
/// This test ensures the default behavior is preserved when the flag is not set.
/// The exhaustive test already covers recheck behavior (line 499), but this explicit test
/// documents the contract more clearly for the restart_on_reentry feature.
#[test]
fn test_reentry_without_restart_uses_recheck() {
    use orkestra_core::workflow::config::{StageCapabilities, StageConfig};

    // Work stage WITHOUT restart_on_reentry (default false)
    let workflow = WorkflowConfig::new(vec![
        StageConfig::new("work", "summary").with_prompt("worker.md"),
        StageConfig::new("review", "verdict")
            .with_prompt("reviewer.md")
    ...
    // =========================================================================
    // Step 1: Complete work → review → integration
    // =========================================================================
    let task = ctx.create_task(
        "Recheck test",
        "Test that default behavior uses recheck on re-entry",
        None,
    );
    let task_id = task.id.clone();
    let base_branch = task.base_branch.clone();
    ...
    // Create integration failure to force pipeline re-entry
    let worktree_path = ctx.repo_path().join(".orkestra/.worktrees").join(&task_id);
    ...
    ctx.advance(); // spawns reviewer
    ctx.advance(); // processes review → integration fails → recovers to work → spawns work agent
    ctx.advance(); // processes work output

    // =========================================================================
    // Step 2: Verify recheck behavior (default without restart_on_reentry)
    // =========================================================================
    ...
    // Verify: should be a RESUME (recheck type)
    let reentry_call = ctx.last_run_config();
    assert!(
        reentry_call.is_resume,
        "Without restart_on_reentry, pipeline re-entry should use resume (recheck)"
    );

    // Verify: prompt should have recheck marker
    ctx.assert_resume_prompt_contains("recheck", &[]);

    // Verify: session was reused (not superseded)
    let final_session = ctx
        .api()
        .get_stage_session(&task_id, "work")
        .unwrap()
        .expect("Work session should still exist");
    assert_eq!(
        final_session.id, original_session.id,
        "Session should be reused (not superseded) without restart_on_reentry"
    );
}
