...
/// `trigger_delivered`, `activity_log`, `gate_result`, `artifact_snapshot`
...
pub fn execute(row: &rusqlite::Row) -> rusqlite::Result<Iteration> {
    ...
    // Column 12 is artifact_snapshot (added in V18 migration)
    let artifact_snapshot_json: Option<String> = row.get(12).unwrap_or(None);
    ...
    Ok(Iteration {
        ...
        artifact_snapshot: artifact_snapshot_json.and_then(|j| serde_json::from_str(&j).ok()),
    })
}
