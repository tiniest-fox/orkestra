...
- **Activity logs**: Agents can output an optional `activity_log` field (short summary string) alongside their main output (artifact/approval/subtasks). The log is persisted on the iteration and injected into prompts for downstream stages via `StagePromptContext.activity_logs`. This provides summarized context of prior work without requiring agents to re-read full artifacts. Activity logs from completed iterations are included in the initial prompt template.
