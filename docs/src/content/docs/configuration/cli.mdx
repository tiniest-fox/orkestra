...
<Callout type="note">
`ork play` runs all stages without pausing for human approval — equivalent to running a Trak in auto mode. Automated gate failures trigger agent retries as usual. `ork play` halts if an agentic gate produces a reject verdict or the agent declares the Trak failed or blocked. Inspect the Trak with `ork trak show` after a failed run.
</Callout>
