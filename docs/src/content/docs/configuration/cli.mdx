...
Initialize Orkestra in the current project. Creates `.orkestra/` with default configuration files.
...
Creates:
- `.orkestra/workflow.yaml` — default 5-stage pipeline (planning, breakdown, work, review, compound)
- `.orkestra/agents/` — prompt templates for each stage (`planner.md`, `breakdown.md`, `worker.md`, `reviewer.md`, `compound.md`)
- `.orkestra/scripts/checks.sh` — gate script template for the work stage

If `.orkestra/` already exists, `ork init` does not overwrite existing files. Running `ork trak list` on a project without `.orkestra/` also bootstraps the directory automatically.
...
| `--description` / `-d` | Yes | Trak description. Describes the work for this Trak. |
...
Additional entry types (gate events, artifact events) may appear in unfiltered output. The `--type` filter accepts only the types listed above.
...
<Callout type="note">
`ork play` runs all stages without pausing for human approval. Automated gate failures trigger agent retries as usual. Agentic gate rejections auto-execute and route back to the target stage. `ork play` halts only if the agent declares the Trak failed or blocked. Inspect with `ork trak show` after a halted run.
</Callout>
