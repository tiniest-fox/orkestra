...
<Callout type="note">
`ork play` runs all stages without pausing for human approval — equivalent to running a Trak in auto mode. `ork play` halts if a gate fails or an agentic gate produces a reject verdict. Inspect the Trak with `ork trak show` after a failed run.
</Callout>
