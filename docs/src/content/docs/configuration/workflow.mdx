...
| `gate` | object | No | — | Shell script that runs after the agent produces output. See [Gate fields](#gate-fields). |
...
| `subtasks` | object | — | Allows the agent to decompose the Trak into child Traks (Subtraks). See [Subtasks](#subtasks-capability). |
| `approval` | object | — | Makes this an Agentic Gate: the agent produces an approve or reject verdict instead of a plain artifact. See [Approval](#approval-capability). |
...
<Callout type="warning" title="capabilities.approval is not the human approval step">
`capabilities.approval` configures an **Agentic Gate** — a stage where an AI agent evaluates the work and produces a verdict. It is distinct from the human approval step that exists on every non-automated stage. See [Capabilities](/docs/core-concepts/capabilities) for the full explanation.
</Callout>
