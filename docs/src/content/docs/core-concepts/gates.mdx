...
A gate is attached to a [stage](/docs/core-concepts/stages) in `workflow.yaml`. Orkestra supports two kinds:
...
Configure an agentic gate by setting `gate: true` on a stage:
