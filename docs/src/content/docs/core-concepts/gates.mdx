...
5. The loop continues until the gate passes. There is no retry limit — the loop is unbounded. To break a stuck gate, use `ork trak skip` to advance past the stage or `ork trak send-to-stage` to reroute.
