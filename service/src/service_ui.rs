...
#[cfg(not(feature = "embed-service-ui"))]
mod inner {
    use axum::response::{Html, IntoResponse};
    use axum::routing::get;
    use axum::Router;

    const MANAGEMENT_HTML: &str = include_str!("../../crates/orkestra-service/src/management.html");

    async fn management_page() -> impl IntoResponse {
        Html(MANAGEMENT_HTML)
    }

    pub fn router() -> Router {
        Router::new().route("/", get(management_page))
    }
}
